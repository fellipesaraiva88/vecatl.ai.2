    ...
    <PageWrapper>
      <IdeasContainer>
        ...
        <InputRow>
          ...
          />
          <AddButton onClick={handleAdd} title="Add Idea">
            <FaPlus />
          </AddButton>
        </InputRow>

        {loading && <EmptyText>Loading ideas...</EmptyText>}

        {!loading && ideas.length === 0 && (
          <EmptyText>No ideas yet. Start writing something 💡</EmptyText>
        )}
        ...
        {ideas.map((idea) => (
          <IdeaCard key={idea.id}>
            ...
            <div>
              <IconButton onClick={() => handleConvertToTask(idea.content, idea.id)} title="Convert to Task">
                <FaTasks />
              </IconButton>
              <IconButton onClick={() => handleConvertToNote(idea.content, idea.id)} title="Convert to Note">
                <FaStickyNote />
              </IconButton>
              <IconButton onClick={() => handleDelete(idea.id)} title="Delete">
                <FaTrash />
              </IconButton>
            </div>
          </IdeaCard>
